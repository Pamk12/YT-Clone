  ...
  playSong(songId: string) {
    ...
    if (data.howl) {
      ...
      this.sound = data.howl = new Howl({
        src: [data.src], // Updated to use the local src passed from PLAYLIST
        html5: true, // Added html5: true to handle large local files smoothly
        volume: this.volume,
        onplay: () => {
          this.trackProgress();
        },
